...
async function checkModMiddleware(req, res, next) {
    try {
        ...
        if(!decoded_data || !decoded_data.isMod){
            return res.status(401).json({message: Error})
        }

        if(user_unxid === decoded_data.user_unx_id && decoded_data.isMod || decoded_data.isAdmin){
            next()
        }

        res.status(401).json({message: 'Failed Moderator Checks!'})
...
module.exports = checkModMiddleware